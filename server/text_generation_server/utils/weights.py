...
class GPTQWeight:
    ...
    # These are generated by the Exllama linear layer, but we probably
    # need to keep them in Python-land.
...
class Exl2Weight:
    ...
    def get_tensor(cls, prefix, weight: "Weights"):
        ...
        for key in Exl2Weight.tensor_keys():
            ...
            name = f"{prefix}.{key}"
            try:
                tensors[key] = weight.get_tensor(name)
            except RuntimeError:
                raise RuntimeError(
                    f"Cannot load `exl2`-quantized weight {name}, make sure the model is already quantized."
                )
